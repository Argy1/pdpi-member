...
export class AnggotaAPI {
  static async getMembers(params: GetMembersParams = {}): Promise<APIResponse<any[]>> {
    try {
      ...
      const adminFields = `${baseFields}, nik, no_str, str_berlaku_sampai, no_sip, sip_berlaku_sampai, no_hp, alamat_rumah, kota_kabupaten_rumah, provinsi_rumah, tempat_lahir, tgl_lahir, keterangan, kota_kabupaten, provinsi, kota_kabupaten_kantor, provinsi_kantor, subspesialis, gelar_fisr, jabatan, website, sosial_media`
      ...
      const dbSortField = sortField === 'nama' ? 'nama' : 
                         ...
                         sortField === 'npa' ? 'npa_numeric' : // Use numeric column for proper sorting
                         sortField === 'createdAt' ? 'created_at' : 'nama'
      
      query = query.order(dbSortField, { 
        ascending: sortDirection === 'asc',
        nullsFirst: false // Put nulls at the end
      })
      ...
      // Apply hospital type filter for count query too - using tempat_praktek_X_tipe_2
      if (hospitalType) {
        const types = hospitalType.split(',').map(t => t.trim()).filter(t => t)
        if (types.length > 0) {
          const hospitalConditions = []
